...
import { useGetTrainingRecipientParticipantsQuery } from "store/api/projectApi";
...
const BulkParticipantsForm = ({
  ...
  // Read from new Redux structure (CQRS pattern)
  ...
  const enrolled = useSelector(selectAllParticipants); // Get from normalized store
  const isCreating = !customer;


  const [selectedEnrollees, setSelectedEnrollees] = useState([]);

  // Use RTK Query to fetch training recipient participants (CQRS Read Model)
  const {
    data: trainingRecipientParticipants = [],
    isLoading: loadingTRParticipants,
    error: trParticipantsError
    ...
    {
      skip: !open || !projectInfo?.trainingRecipientId, // Skip if dialog closed or no training recipient
    }
  );
  ...
  // Filter out participants that are already enrolled
  // The transferLists component already filters based on isEnrolled flag,
  // but we double-check here to prevent duplicate constraint errors
  const filteredSelectedEnrollees = selectedEnrollees.filter(selectedEnrollee => {
    // If the participant has isEnrolled flag, use that as source of truth
    if (selectedEnrollee.hasOwnProperty('isEnrolled')) {
      return !selectedEnrollee.isEnrolled;
    }
    // Fallback: check if they have a projectId (old logic)
    return typeof selectedEnrollee.projectId === 'undefined';
  });
        ...
        if (customer) {
          ...
          if (!values.role) {
            dispatch(openSnackbar({
              open: true,
              message: "Please select a role for the participant",
              variant: "alert",
              alert: { color: "error" },
              close: false,
            }));
          ...
          await onAdd(newParticipant);
        } else {
          // Add multiple participants using the bulk command
          // filteredSelectedEnrollees already contains the participants in correct format

          if (filteredSelectedEnrollees.length === 0) {
            dispatch(openSnackbar({
              open: true,
              message: "Please select at least one participant to enroll",
              variant: "alert",
              alert: { color: "warning" },
              close: false,
            }));
          ...
          await onAdd({
            participants: filteredSelectedEnrollees,
            isMultiple: true
          });
        }

        setSubmitting(false);
        // Note: onCancel() is removed - let the parent handler close the dialog on success
      } catch (error) {
        setSubmitting(false);
        dispatch(openSnackbar({
          open: true,
                  ...
                  <Grid item xs={12}>
                    <Stack spacing={1.25}>
                      <InputLabel htmlFor="group-participants">
                        Enroll existing participants from {projectInfo?.training_recipient?.name || 'training recipient'}
                      </InputLabel>
                      {!projectInfo?.trainingRecipientId ? (
                        <Alert severity="warning" sx={{ mt: 2 }}>
                          No training recipient assigned to this project. Please assign a training recipient in the Project Settings tab.
                        </Alert>
                      ) : loadingTRParticipants ? (
                        <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}>
                          <CircularProgress />
                          <Typography sx={{ ml: 2 }}>
                            Loading participants from {projectInfo?.training_recipient?.name}...
                          </Typography>
                        </Box>
                      ) : trParticipantsError ? (
                        <Alert severity="error" sx={{ mt: 2 }}>
                          Failed to load participants from {projectInfo?.training_recipient?.name}
                          <br />
                          Error: {trParticipantsError?.message || 'Unknown error'}
                        </Alert>
                        ...
                        <Alert severity="info" sx={{ mt: 2 }}>
                          No participants found for "{projectInfo?.training_recipient?.name || 'this training recipient'}".
                          <br />
                          The training recipient may not have any participants yet, or all participants are already enrolled in this project.
                          <br />
                          You can add new participants using the "Add Single Participant" toggle above.
                        </Alert>
                      )}
                    </Stack>
                  </Grid>
...
BulkParticipantsForm.propTypes = {
...
export default BulkParticipantsForm;