...
} from 'store/reducers/project/agenda';
import { openSnackbar } from 'store/reducers/snackbar';

/**
 * Custom hook for managing participant attendance operations
 * Handles attendance status updates, participant removal, and group movement
 */
export const useAttendanceManagement = (eventParticipants, selectedEvent, singleProject) => {
  ...
  const handleStatusChange = useCallback(async (participantId, newStatus) => {
    ...
    try {
      if (eventParticipant && eventParticipant.isDirect === false) {
        ...
        await dispatch(addEventParticipant(selectedEvent.id, participantId, newStatus));
      } else {
        ...
        await dispatch(updateAttendanceStatus(selectedEvent.id, enrolleeId, newStatus));
      }
  ...
  const handleRemoveFromEvent = useCallback(async (participantId, participant) => {
    ...
    try {
      await dispatch(removeEventParticipant(selectedEvent.id, participantId));
      dispatch(openSnackbar({
        open: true,
  ...
  const handleMoveToGroup = useCallback(async (participantId, participant, targetGroup) => {
    ...
    try {
      ...
      await dispatch(moveParticipantToGroup(participantId, currentGroup?.id, targetGroup.id));
