...
const TrainingRecipients = () => {
  ...
  // Filter training recipients based on archived status
  const filteredRecipients = useMemo(() => {
    if (!trainingRecipients) return [];
    return trainingRecipients.filter(recipient => {
      if (showArchived) {
        return recipient.status === 'archived';
      }
      return recipient.status === 'active';
    });
  }, [trainingRecipients, showArchived]);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await dispatch(deleteTrainingRecipient(recipientToDelete.id));
  ...
  const handleArchive = async () => {
    ...
    try {
      // Update the training recipient status to 'archived'
      await dispatch(updateTrainingRecipient({
      ...
      dispatch(openSnackbar({
        open: true,
        message: 'Training recipient archived successfully',
        variant: 'alert',
        alert: { color: 'success' },
        close: false
      }));
      ...
      console.error('Error archiving training recipient:', error);
    }
  };
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      Cell: ({ row }) => {
        ...
        const participantCount = row.original._count?.participants || 0;
        ...
        return (
          <Stack spacing={0.5}>
            ...
            <Stack direction="row" alignItems="center" spacing={0.5}>
              ...
              <Typography variant="body2">
                {participantCount} Total Participant{participantCount !== 1 ? 's' : ''}
              </Typography>
            </Stack>
          </Stack>
        );
    ...
    <Page title="Training Recipients & Participants">
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        ...
        <FormControlLabel
          ...
          label={showArchived ? "Showing Archived" : "Showing Active"}
          sx={{ ml: 2 }}
        />
      </Stack>
          ...
          <ScrollX>
            {loading ? (
              ...
              <AppTable
                ...
                emptyMessage={showArchived ? "No archived training recipients found" : "No active training recipients found"}
      ...
      <DeleteCard
        ...
        showArchive={true}
        title="Delete or Archive Training Recipient"
        itemName={recipientToDelete?.name}
        message={`What would you like to do with "${recipientToDelete?.name}"? Archive will hide it from the list but keep all data. Delete will permanently remove it.`}
