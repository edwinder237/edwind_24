...
const FullCalendarMonthViewCQRS = ({ project, events, onEventSelect }) => {
  const theme = useTheme();
  const dispatch = useDispatch();

  // CQRS: Fetch agenda data using RTK Query (auto-updates entity stores)
  const { data: agendaData, isLoading: isLoadingAgenda, refetch: refetchAgenda } = useGetProjectAgendaQuery(
    project?.id,
    {
      skip: !project?.id,
  ...
  const handleEventDrop = useCallback(async (dropInfo) => {
    ...
    try {
      ...
      // CQRS: Refresh agenda data using RTK Query
      if (project?.id) {
        await refetchAgenda();
      }
...
export default FullCalendarMonthViewCQRS;